...
//            attachView(header, findLastIndexForSection(sd.firstPosition) + 1);
    ...
    private StickyHeaderWatcher watcher;
     ...
     * @@@@@@@@@not implements @@@@@@@@@@@@@@@@@
     */
    /*private void setStickyHeaderWatcher(int watchDistance, StickyHeaderWatcher watcher){
     ...
     * @param watcher Watcher.
     */
    public void setOnHeaderListener(StickyHeaderWatcher watcher){
        this.watcher = watcher;
    }
...
//        void onPositionChange(View header, int position, int distance);
         ...
         * @param position View position in adapter data set.
         */
        void onStickyStateChanged(View header, int position );
    }
    private  View currentFirstView ;
    private void watcherHeader() {
        if (getChildCount() != 0) {
            final View child = getChildAt(0);
            final LayoutParams params = (LayoutParams) child.getLayoutParams();
            final int sfp = params.getTestedFirstPosition();
            ...
            final LayoutParams firstParams = (LayoutParams) first.getLayoutParams();
            if (firstParams.isHeader) {
                if (currentFirstView != first){
                    currentFirstView = first;
                    int position =  getPosition(currentFirstView);
                    if(watcher != null){
                        watcher.onStickyStateChanged(currentFirstView, position);
                    }
//                    Log.i("fillEdge getAtStart", ((TextView) first).getText().toString() + " getPosition " + position);
                }
            }
        }
    }

    public View getCurrentFirstView() {
        return currentFirstView;
    }
